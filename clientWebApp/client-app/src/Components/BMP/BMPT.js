...
class BMPT extends Component {
    render() {
        return (
          <div className="component">
              <Card className ='grey lighten-3' textClassName='grey-text' title='BMPT'>
    ...
    componentWillMount(){
        var that = this
        this.props.socket.on('BNOA', ({temp})=>{
          that.setState({
            x: temp,
            tempPoints: [...that.state.tempPoints.slice(-10),x], //that.state.Xpoint.push(x)
